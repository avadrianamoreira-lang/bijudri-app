const SUPABASE_URL = "[kzlvxawotfnxjbnhysdx.supabase.co](https://kzlvxawotfnxjbnhysdx.supabase.co)";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt6bHZ4YXdvdGZueGpibmh5c2R4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU4MDQ4MzYsImV4cCI6MjA5MTM4MDgzNn0.Kh5ugtSRLQXLbsM2JXzKJEv85L5H64IStftuLNfNd8";
...
async function createProduct() {
  ...
  const unique = await ensureUniqueName(payload.name);
  if (!unique) {
    renderProductFormError("Ja existe um produto com este nome.");
    return;
  }
...
async function updateProduct(id) {
  ...
  const unique = await ensureUniqueName(payload.name, id);
  if (!unique) {
    renderProductFormError("Ja existe outro produto com este nome.");
    return;
  }
...
async function toggleProductFeatured(id, featured) {
  const { error } = await supabaseClient.from("products").update({ featured }).eq("id", id);
  if (error) {
